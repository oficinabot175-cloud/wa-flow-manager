...
class WaApi {
  constructor() {
    ...
    this.token = sessionStorage.getItem('wa_app_token') || '';
  }
  ...
  setToken(token) {
    this.token = token;
    sessionStorage.setItem('wa_app_token', token);
  }
  ...
  async request(action, params = {}) {
    if (!this.baseUrl) throw new Error('Backend URL not configured. Go to Settings.');

    const url = new URL(this.baseUrl);
    url.searchParams.set('action', action);
    if (this.token) url.searchParams.set('token', this.token);

    // For simple GET-like requests, use query params
